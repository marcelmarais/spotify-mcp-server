...
for (const mode of ['legacy', { pin: '2026-07-28' }]) {
  test(`discovers all tools and validates arguments (${JSON.stringify(mode)})`, async (t) => {
    ...
    for (const [name, args] of [
      ...
      ['removeUsersSavedTracks', { trackIds: Array(5001).fill('track') }],
      ['saveTracksToLibrary', { trackIds: Array(5001).fill('track') }],
      ['saveTracksToLibrary', { trackIds: [] }],
      ['checkUsersSavedTracks', { trackIds: Array(5001).fill('track') }],
