			...
			try {
				...
				if (spotifyUri) {
					...
					if (isTrack) {
						...
						await spotifyApi.player.startResumePlayback(targetDeviceId, spotifyUri);
					}
				} else {
				...
				if (spotifyUri) {
					...
					if (isTrack) {
						await spotifyApi.player.startResumePlayback(selected.id, undefined, [
							spotifyUri,
						]);
					} else {
						await spotifyApi.player.startResumePlayback(selected.id, spotifyUri);
					}
				} else {
...
const addToQueue: tool<{
	uri: z.ZodOptional<z.ZodString>;
	type: z.ZodOptional<z.ZodEnum<['track', 'album', 'artist', 'playlist', 'radio']>>;
	id: z.ZodOptional<z.ZodString>;
	deviceId: z.ZodOptional<z.ZodString>;
}> = {
	name: 'addToQueue',
